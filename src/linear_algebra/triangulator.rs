...
fn is_ear(poly: &Polygon, prev: &Vertex, ear: &Vertex, next: &Vertex) -> bool {
    // Check winding
    if super::get_signed_triangle_area(prev.position, ear.position, next.position) >= 0.0 {
        return false;
    }
    ...
    loop {
        ...
        if i != prev.index
            && i != ear.index
            && i != next.index
            && super::is_point_inside_2d_triangle(
                vertex.position,
                prev.position,
                ear.position,
                next.position,
        ...
        if let Ok(normal) = super::get_polygon_normal(vertices) {
            ...
            let mut polygon = Polygon {
                vertices: vertices
                    ...
                    .enumerate()
                    .map(|(i, point)| Vertex {
                        position: super::vec3_to_vec2_by_plane(plane_class, normal, *point),
                        index: i,
